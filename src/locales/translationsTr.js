const translationsTr = {
  gallery: "Ana Sayfa",
  arts: "Sanat Eserleri",
  about: "Blog",
  contact: "İletişim",

  hero_text1: "BUHARA MİNİYATÜR OKULUNUN EN PARLAK TEMSİLCİSİ",
  hero_name: "TOSHEV DAVLAT BORONOVICH",
  hero_text2: "MİNİYATÜR USTASI",
  hero_text3: "NAKKAŞ",
  hero_text4: "HAT SANATÇISI",

  productH3: "Minyatür Koleksiyonu",
  productP: "Minimalist, zarif ve galeri kalitesinde sanat eserleri.",

  product1_name: "SÜLEYMAN (A.S.)",
  product1_info: "Minyatür sanatının modern yorumu",

  product2_name: "DÜĞÜN TÖRENİ",
  product2_info: "Özbek düğün geleneklerinden ilham alan zarif bir minyatür",

  product3_name: "ŞÖLEN",
  product3_info: "Kutlama ve neşenin sanatsal ifadesi",

  product4_name: "ŞEMSÜL KAMER",
  product4_info: "Işığın ve güzelliğin sembolü",

  product5_name: "20×27",
  product5_info: "Küçük ölçekte zarafet",

  product6_name: "18×29",
  product6_info: "Sadelikte estetik uyum",

  product7_name: "34.5×49",
  product7_info: "Büyük formatta sanatsal kompozisyon",

  product8_name: "15×25",
  product8_info: "Minyatür formda mükemmellik",

  aboutUsP: `Toshev Davlat Bo‘ronovich, Buhara’da geleneksel uygulamalı sanatlar ve minyatür sanatını sürdüren bir ressam, nakkaş ve hattattır. Kendisinin kurduğu “Ustoz-Şogird Uygulamalı Sanatlar Merkezi”nde gençler, engelli çocuklar dâhil, ücretsiz eğitim almakta ve millî değerlere saygı ruhunda yetiştirilmektedir.
Onun sanatının özü, Yaratıcıya duyulan sevgi ve O’nu anlama arzusudur. Eserlerinde evrensel insani değerler ile İslam dininin barışçıl ve hikmet dolu özünü yansıtmaktadır.
2025 yılında Özbekistan Cumhuriyeti Cumhurbaşkanı kararnamesiyle “Şuhrat” Madalyası ile ödüllendirilmiştir. Günümüzde ise öğrencileriyle birlikte Buhara’nın zengin sanat geleneklerini çağdaş sanatsal arayışlarla uyum içinde sürdürmektedir.`,
  aboutUsSpan: "Usta-Çırak Merkezi Kurucusu",
  managerName: "Davlat Toshev",

  contactP: "Satın alma talebiniz hakkında kısa bir not bırakın. Gerisini biz hallederiz.",
  contactNameInp: "İsim",
  contactPhoneInp: "Telefon:",
  contactEx: "Sadece rakam girin. Örnek: 90 123 45 67",
  contactBtn: "Gönder",
};

export default translationsTr;
